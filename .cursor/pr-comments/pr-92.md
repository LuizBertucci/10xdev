...
**Titulo**: Refatorar UI de projetos e códigos com melhorias de visibilidade e ownership
...
|---|------------|---------|----------|--------|
| 1 | 🔴 Critical | CardFeatureModel.ts:87-109 | Admin nao consegue mais listar cards pendentes/rejeitados de outros usuarios para moderacao | ✅ Resolvido |
| 2 | 🟠 Major | ProjectModel.ts:483-511 | Comentarios do backend dizem CASCADE, mas FK e SET NULL - cards sao orfaos, nao deletados | |
| 3 | 🟠 Major | Projects.tsx:370-398 | Texto do dialog contradiz o checkbox quando CASCADE esta em efeito | ✅ Resolvido |
| 4 | 🟡 Minor | .claude/settings.local.json:1-18 | Arquivo local que nao deveria ser commitado | |
...
| 10 | Nitpick | CardFeatureModel.ts:214-216, 877-879 | Ternary tautologico - ambas branches sao Visibility.UNLISTED | ✅ Resolvido |

## Detalhamento dos Actionable Comments

### 1. 🔴 Critical: CardFeatureModel.ts - Admin nao consegue moderar
...
+
+      // Admin também vê todos os pendentes (para moderação na aba "Validando")
+      if (userRole === 'admin') {
...
### 2. 🟠 Major: ProjectModel.ts - CASCADE vs SET NULL
...
### 3. 🟠 Major: Projects.tsx - Dialog contradiz checkbox
...
### 4. 🟡 Minor: .claude/settings.local.json - Arquivo local commitado
...
**Acao**: Atualizar menção a "Seu Espaço" para referenciar apenas `visibility='unlisted'`.

### 2. .gitignore

**Problema**: `.claude/settings.local.json` ainda está sendo trackeado apesar de estar no .gitignore.
...
- Linha ~352-371: Quando handling `Visibility.UNLISTED`,短路 requests nao autenticados antes de chamar `.eq('shared_with_user_id', userId)`
...
**Acao**: Adicionar verificação responsiva para fechar apenas no mobile (`useMediaQuery('md')`).
...
**Acao**: Remover o unreachable JSX ou mudar lógica para permitir array vazio.
