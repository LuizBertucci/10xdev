 ...
 */
export function groupCardsByMacroCategory(cards: any[]): Record<string, any[]> {
  ...
  // Agrupa os cards
  cards.forEach(card => {
    const macroCategory = getMacroCategory(card.tags)
  ...
  Object.entries(MACRO_CATEGORIES)
    ...
    .forEach(([macroCategory]) => {
      if (grouped[macroCategory].length > 0) {
 ...
 */
export function getMacroCategoryStats(cards: any[]): Array<{
  category: string
  count: number
  description: string
  priority: number
